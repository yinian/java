 ...
 *
 * @author yinian 2015-08-26 21:36:43
 */
...
//		getClient();    // async coon, support init without conn
	...
	public ZooKeeper getClient(){
		...
		if (zooKeeper == null){
			try {
				if (INSTANCE_INIT_LOCK.tryLock(2, TimeUnit.MILLISECONDS)){
					...
					try {
						if (zooKeeper == null){// 二次校验，防止并发创建client
							...
							newZk.exists(zkpath, false); // sync wait until success conn
	...
	public Map<String, String> getChildPathData(String path){
		...
		try {
			Stat stat = getClient().exists(path, true);// 获取子节点的状态
			if (stat == null) {
